...
bool dynamixel_sdk_custom_interfaces__msg__set_position__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // position
    ...
    int32_t * dest = ros_message->position.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      PyObject * item = PySequence_Fast_GET_ITEM(seq_field, i);
      if (!item) {
        Py_DECREF(seq_field);
...
PyObject * dynamixel_sdk_custom_interfaces__msg__set_position__convert_to_py(void * raw_ros_message)
{
  ...
  {  // position
    ...
    if (ros_message->position.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int32_t * src = &(ros_message->position.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->position.size * sizeof(int32_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
